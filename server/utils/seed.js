    ...
    if (productsCount >= NUM_PRODUCTS) {
      ...
      for (let i = 0; i < NUM_PRODUCTS; i++) {
        ...
        try {
          ...
          if (!res.ok || !data.urls) {
            throw new Error('Unsplash API returned invalid data');
          }

          imageUrl = data.urls.small || data.urls.regular || '';
          imageKey = data.id || '';
        } catch (err) {
          console.error('Unsplash failed, trying Pixabay:', err.message);

          try {
            const res = await fetch(
              `https://pixabay.com/api/?key=${
                process.env.PIXABAY_API_KEY
              }&q=${encodeURIComponent(name)}&image_type=photo&per_page=3`
            );
            const data = await res.json();

            if (res.ok && data.hits && data.hits.length > 0) {
              imageUrl =
                data.hits[0].webformatURL || data.hits[0].largeImageURL || '';
              imageKey = data.hits[0].id?.toString() || '';
            } else {
              throw new Error('Pixabay returned no usable image');
            }
          } catch (err) {
            console.error('Pixabay also failed:', err.message);
            imageUrl = ''; // fallback to empty or placeholder if needed
            imageKey = '';
          }
        }
