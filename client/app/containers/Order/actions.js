...
export const addOrder = address => {
  return async (dispatch, getState) => {
    const response = await axios.post(`${API_URL}/payment/payment-config`);
    ...
    try {
      ...
      const { id: razorpayOrderId, amount } = razorpayOrder.data;

      // Step 2: Open Razorpay checkout
      const options = {
        key: config.key_id, // replace with your Razorpay key ID
        amount: amount,
        currency: 'INR',
        name: 'CARTINDIA',
        ...
        handler: async function (response) {
          ...
          dispatch(push(`/order/success/${orderResponse.data.order._id}`));
          dispatch(clearCart());
        },
        prefill: {
